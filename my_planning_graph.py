...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for pg_node_a in self.a_levels[level - 1]:
            for pg_node_s in pg_node_a.effnodes:
                self.s_levels[level].add(pg_node_s)
                pg_node_s.parents.add(pg_node_a)
                pg_node_a.children.add(pg_node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # check node_a1's positive effect and node_a2's negative effect, and the opposition.
        ...
        return len(a1_pos_effect.intersection(a2_neg_effect)) > 0 or \
            len(a1_neg_effect.intersection(a2_pos_effect)) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a2_pos_effect = set(node_a2.action.effect_add)
        a2_neg_effect = set(node_a2.action.effect_rem)
        return len(a1_pos_effect.intersection(a2_neg_precond)) > 0 or \
                   len(a1_neg_effect.intersection(a2_pos_precond)) > 0 or \
                   len(a2_pos_effect.intersection(a1_neg_precond)) > 0 or \
                   len(a2_neg_effect.intersection(a1_pos_precond)) > 0
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        # check whether any non-mutex actions exist
        for s1_parent_node in node_s1.parents:
            for s2_parent_node in node_s2.parents:
                if not s1_parent_node.is_mutex(s2_parent_node):
                    return False
        return True
        ...
        for goal in self.problem.goal:
            for level, s_nodes in enumerate(self.s_levels):
                for s_node in s_nodes:
                    ...
                    # not found same literal as goal, continue to check next level's s_nodes
                    continue
                # found same literal as goal, then check next goal
                break
        return level_sum
